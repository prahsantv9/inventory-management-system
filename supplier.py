...
class supplierclass:
    def __init__(self,root):
        ...
        #=========option=========
        lbl_search=Label(SearchFrame,text="Search By Invoicd No",bg="white",font=("goudy old style",15))
        ...
        txt_supplier_invoce=Entry(self.root,textvariable=self.var_sup_invoice,font=("gaudy old style",15),bg="lightyellow").place(x=150,y=150,width=180)
        ...
        try:
            if self.var_sup_invoice.get()=="":
               ...
               if row==None:
                    ...
                    cur.execute("Update suppplier set name=?,contact=?,desc=? where invoice=?",(
    ...
    def search(self):    
        ...
        try:
            if self.var_emp_searchtxt.get()=="":
                ...
                cur.execute("select * from supplier where invoice=?",(+self.var_emp_searchtxt.get(),))
                row=cur.fetchone()
                if row!=None:
                    self.supplierTable.delete(*self.supplierTable.get_children())
                    self.supplierTable.insert('',END,values=row)
